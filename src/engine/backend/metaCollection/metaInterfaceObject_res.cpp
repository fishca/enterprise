#include "metaInterfaceObject.h"

/* PNG */
static const wxString s_interface_16_png = wxT("");

wxIcon CValueMetaObjectInterface::GetIcon() const
{
	return GetIconGroup();
}

wxIcon CValueMetaObjectInterface::GetIconGroup()
{
	static wxIcon icon =
		CBackendPicture::GetIconFromBase64(s_interface_16_png, wxSize(16, 16));

	return icon;
}
