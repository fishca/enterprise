#include "metaModuleObject.h"

/* PNG */
static const wxString s_module_64_png = wxT("");

wxIcon IValueMetaObjectModule::GetIcon() const
{
	return GetIconGroup();
}

wxIcon IValueMetaObjectModule::GetIconGroup()
{
	static wxIcon icon =
		CBackendPicture::GetIconFromBase64(s_module_64_png, wxSize(16, 16));

	return icon;
}

wxIcon CValueMetaObjectCommonModule::GetIcon() const
{
	return GetIconGroup();
}

wxIcon CValueMetaObjectCommonModule::GetIconGroup()
{
	static wxIcon icon =
		CBackendPicture::GetIconFromBase64(s_module_64_png, wxSize(16, 16));

	return icon;
}

wxIcon CValueMetaObjectManagerModule::GetIcon() const
{
	return GetIconGroup();
}

wxIcon CValueMetaObjectManagerModule::GetIconGroup()
{
	static wxIcon icon =
		CBackendPicture::GetIconFromBase64(s_module_64_png, wxSize(16, 16));

	return icon;
}
