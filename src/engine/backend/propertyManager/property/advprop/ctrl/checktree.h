#ifndef __PROPERTY_CHECK_TREE_H_INCLUDED__
#define __PROPERTY_CHECK_TREE_H_INCLUDED__
...
#define wxPR_EMPTY_CHECK 0x1600
#define wxPR_SINGLE_CHECK 0x2000
#define wxPR_MULTIPLE_CHECK 0x4000

#include "backend/backend.h"

class wxPropertyCheckTree : public wxTreeCtrl {
	bool m_singleCheck;
	bool m_allowEmpty;
...
protected:
		...
		wxTreeItemData* data)
	{
		...
		//wxPropertyCheckTree::Check(id, false);
...
public:
	...
	wxPropertyCheckTree(wxWindow* parent, const wxWindowID id,
		const wxPoint& pos, const wxSize& size,
		long style);
	wxPropertyCheckTree();
	virtual ~wxPropertyCheckTree();
	...
	{
		m_singleCheck = (style & wxPR_MULTIPLE_CHECK) != wxPR_MULTIPLE_CHECK;
		...
		if (m_singleCheck) {
			...
			for (auto& sel : m_checkedItems) {
				if (sel.second) {
					if (!firstSel) {
						wxPropertyCheckTree::SetItemState(sel.first, wxPropertyCheckTree::UNCHECKED);
						sel.second = false;
					}
					else {
						wxPropertyCheckTree::SelectItem(sel.first);
					}
					firstSel = false;
				}
			}
...
private:
	...
	wxDECLARE_DYNAMIC_CLASS(wxPropertyCheckTree);
};

wxDECLARE_EVENT(wxEVT_PROPERTY_CHECKTREE_CHOICE, wxTreeEvent);
wxDECLARE_EVENT(wxEVT_PROPERTY_CHECKTREE_FOCUS, wxTreeEvent);
